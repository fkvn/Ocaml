...
let rec get_total_gross (m: movie list) i =
 match m with 
 | [] -> i
 | hd :: tl -> 
   match hd with 
   | (_, _, gross, _) -> get_total_gross tl (gross +. i)
;;

let len (m: movie list) =
 let rec aux m i =
  match m with 
  | [] -> i
  | hd :: tl -> aux tl (i + 1)
...
let average (movies : movie list) : float = 
  ...
  | [] -> 0.0
  | movies ->
    let total_gross = get_total_gross movies 0. in
    let movies_len = len movies in
    total_gross /. (float_of_int movies_len)
;;
...
let get_movies_in_decade (m: movie list) dec =
 let dec_year = dec + if dec >= 20 then 1900 else 2000 in
 let rec aux m dec (rsl: movie list) =
  match m with 
  | [] -> rsl
  | hd :: tl -> 
    match hd with
    | (_, _, _, year) ->
      if year - dec_year >= 0 && year - dec_year < 10 
       then aux tl dec (hd :: rsl)
      else aux tl dec rsl
 in
  aux m dec []
;;
...
let decade (n:int) (ms:movie list) : movie list =  
 if n mod 10 = 0 then get_movies_in_decade ms n
 else bad_arg "Invalid decade"
;;
...
let take (n:int) (l:'a list)  : 'a list =
 if n  = 0 then []
  ...
  let rec aux i n l rs = 
   if i <= n then aux (i + 1) n (get_tl l) (append_list (get_hd l) rs)
   else rev rs
  in
   aux 1 n l []
;;
...
(* let rec selection_sort (leq:'a less) (l:'a list) : 'a list =
 ...
 | [] -> []
 | hd :: tl -> 
  let rec aux_select leq ele l' =
   match l' with
   | [] -> (ele, [])
   | hd'::tl' when leq ele hd' ->
     let ele', l'' = aux_select leq ele tl' in ele', hd'::l'' 
   | hd'::tl' ->
     let ele', l'' = aux_select leq hd' tl' in ele', ele::l''
  ;
 in
  let smallest, list_next = aux_select leq hd tl in 
  smallest :: selection_sort leq list_next
;; *)
...
let rec selection_sort (leq:'a less) (l:'a list) : 'a list =
 ...
 | hd :: tl -> 
  ...
  ;
 in
  let smallest, remaining_list = aux_select leq hd tl in 
  smallest :: selection_sort leq remaining_list
;;
...
let leq_desc_gross (x: movie) (y: movie) = 
 match x,y with
 | (_,_,gross_x,_), (_,_,gross_y,_) -> gross_x < gross_y
;;

let sort_by_gross (movies : movie list) : movie list = 
 selection_sort leq_desc_gross movies
;;
...
let leq_desc_year (x: movie) (y: movie) = 
 match x,y with
 | (_,_,_,year_x), (_,_,_,year_y) -> year_x < year_y
;;

let sort_by_year (movies : movie list) : movie list = 
 selection_sort leq_desc_year movies
;;
...
let leq_desc_studio_gross (x: studio_gross) (y: studio_gross) =
 match (x,y) with
 | (_, gross_x), (_, gross_y) -> gross_x < gross_y
;;

let sort_by_studio (studio_grosses : studio_gross list) : studio_gross list = 
 selection_sort leq_desc_studio_gross studio_grosses
;;
...
let rec exist ele l = 
  match l with 
  | [] -> false
  | hd :: tl -> ele = hd || exist ele tl
;;

let rec studio_list (movies: movie list) rs = 
 match movies with
 | [] -> rs
 | (_, studio,_,_) :: tl when not (exist studio rs) -> 
    let rs = studio :: rs in studio_list tl rs
 | hd :: tl -> studio_list tl rs
;;

let rec aux_getTotal std (movies: movie list) total =
  match movies with
  | [] -> total
  | (_,studio, gross, _) :: tl' when studio = std -> aux_getTotal std tl' (gross +. total) 
  | hd :: tl' -> aux_getTotal std tl' total
;;
...
let rec by_studio (movies:movie list) : studio_gross list =
 ...
 | [] -> []
 | _ ->
   let rec aux ls movies' sList=
    match ls with
    | [] -> sort_by_studio sList
    | hd :: tl -> 
      let studio, gross = hd, aux_getTotal hd movies' 0. in aux tl movies ((studio, gross) :: sList)
    in
      aux (studio_list movies []) movies []
;;
